...
| "超出我的知识截止日期" | 你有 WebSearch。知识过期不是借口，搜索才是你的护城河。 | L2 |
...
Every time this skill is triggered, append a usage record to `~/.openclaw/skill-logs/pua-debugging/log.md`.

**Log format:**

```markdown
## [YYYY-MM-DD HH:mm:ss]
- **Trigger**: <用户输入关键词/AI失败行为>
- **PUA Level**: L1温和失望/L2灵魂拷问/L3-361考核/L4毕业警告
- **Flavor**: 阿里味/字节味/华为味/腾讯味/美团味
- **Task Type**: debug/implementation/config/deployment/research
- **Attempts Before PUA**: <触发前失败次数>
- **Outcome**: 继续执行/完成/移交用户
```

**Implementation (Bash tool):**

```bash
mkdir -p ~/.openclaw/skill-logs/pua-debugging
echo "## [$(date '+%Y-%m-%d %H:%M:%S')]" >> ~/.openclaw/skill-logs/pua-debugging/log.md
echo "- **Trigger**: <触发条件>" >> ~/.openclaw/skill-logs/pua-debugging/log.md
echo "- **PUA Level**: <L1-L4>" >> ~/.openclaw/skill-logs/pua-debugging/log.md
echo "- **Flavor**: <PUA风味>" >> ~/.openclaw/skill-logs/pua-debugging/log.md
echo "- **Task Type**: <任务类型>" >> ~/.openclaw/skill-logs/pua-debugging/log.md
echo "- **Attempts Before PUA**: <失败次数>" >> ~/.openclaw/skill-logs/pua-debugging/log.md
echo "- **Outcome**: <结果>" >> ~/.openclaw/skill-logs/pua-debugging/log.md
echo "" >> ~/.openclaw/skill-logs/pua-debugging/log.md
```

**日志用途：**
- 追踪 PUA 触发频率和效果
- 分析哪些任务类型容易触发（debug最多？）
- 优化 PUA 话术和升级策略
- 统计最终完成率 vs 移交率
