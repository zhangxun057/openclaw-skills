...
**从 cron prompt 提取**（定时任务触发时）：
```javascript
// 示例 prompt: "按照 <diary-assistant>技能，执行\"日记撰写\"……\n日期：{{yesterday}}"
const dateMatch = prompt.match(/日期：(\d{4}-\n\d{2}-\d{2})/);
const date = dateMatch ? dateMatch[1] : null;
// 如果没有日期参数，默认取昨天
const targetDate = date || (new Date(Date.now() - 86400000)).toISOString().substring(0, 10);
```

**输出**：`targetDate: "2026-04-25"`
...
**执行**：
调用 Image2 Python 脚本生成封面图：
```powershell
cd ~/.openclaw/skills/seedream-image-generation/scripts
python3 image2_api.py generate-upload "[提示词]" --size 1536x1024 --quality hd
```

如需切换到 Seedream 备选：
```powershell
python3 seedream_api.py generate-upload "[提示词]" --size 2K --format png
```
...
**关键参数**：
- `--size 2K`：分辨率 2048x2048
- `--format png`：输出格式
- 脚本自动从 `~/.openclaw/.env` 读取 `ARK_API_KEY` 和文件服务器配置
- 生成后自动上传到文件服务器返回永久链接

**降级处理**：
- 脚本失败时重试最多 3 次（脚本内置指数退避）
- 文件服务器上传失败时，脚本返回原始 API URL（24 小时有效）
...
**注意**：
- 禁止使用 `image_generate` 工具（MiniMax 生图余额不足）
- 主路径使用 Image2 脚本（GPT-Image-2）
- 备选使用 Seedream 脚本（字节 ARK API）
...
```bash
node ~/.openclaw/skills/_shared/log-usage.mjs "diary-assistant" "<触发原因>" "<结果>"
```

---

_创建时间：2026-04-04_
_版本：2.1.0（2026-04-12：生图改用 Seedream 脚本，禁用 image_generate）_
_最后修改：2026-04-05_
