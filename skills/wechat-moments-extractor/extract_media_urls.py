...
# -*- coding: utf-8 -*-
"""
[已废弃] 从已分析的 JSONL 数据中筛选值得分析图片的帖子

⚠️ 本脚本已废弃，请使用 run_batch.py + analyze_media.py

旧逻辑：读取 logs/ JSONL，手动规则筛选值得分析的帖子，输出 valuable_posts.json
新逻辑：run_batch.py 提取时直接输出 image_trigger 字段，analyze_media.py 读取 trigger=1
