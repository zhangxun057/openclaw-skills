...
def merge_batch_results(all_dimension_results, posts, batch_size, whitelist):
    ...
    for batch_idx in range(num_batches):
        ...
        sg_batch = all_dimension_results['signals'][batch_idx] or []
        ...
        for i in range(len(ev_batch)):
            ...
            # signals 结果（可能是 dict 或 list）
            sg_item = sg_batch[i] if i < len(sg_batch) else {'signals': [], 'image_prompt': None}
...
def run_full_analysis(posts_file, batch_size=10, output_file=None, max_workers=10, max_concurrent_batches=30):
    ...
    # 写入 logs/YYYY-MM-DD.jsonl
    write_logs(final_results)

    # 更新 profiles
    update_profiles(final_results)
...
def write_logs(results):
    """将分析结果写入 logs/YYYY-MM-DD.jsonl（跳过噪声和已有 post_id）"""
    log_path = os.path.join(LOG_DIR, f"{datetime.now().strftime('%Y-%m-%d')}.jsonl")
    existing_ids = set()
    ...
    with open(log_path, 'a', encoding='utf-8') as f:
        for r in results:
            ...
            if not pid or pid in existing_ids:
                continue
            # 只写有信息量的记录（有 events/traits/signals 任一非空）
            if not (r.get('events') or r.get('traits') or r.get('signals')):
                continue
            entry = {k: v for k, v in r.items() if k != '_failed_fields'}
            f.write(json.dumps(entry, ensure_ascii=False) + '\n')
            written += 1
    print(f"[LOG] 写入 {written} 条 → {log_path}")


def update_profiles(results):
    """按 wxid 将 log 记录追加到 profiles/wxid_xxx.jsonl"""
    os.makedirs(PROFILES_DIR, exist_ok=True)
    by_wxid = {}
    for r in results:
        wxid = r.get('wxid', '')
        if not wxid or not (r.get('events') or r.get('traits') or r.get('signals')):
            continue
        by_wxid.setdefault(wxid, []).append(r)
    updated = 0
    for wxid, records in by_wxid.items():
        profile_path = os.path.join(PROFILES_DIR, wxid + '.jsonl')
        existing_ids = set()
        ...
        with open(profile_path, 'a', encoding='utf-8') as f:
            for r in records:
                if r.get('post_id') not in existing_ids:
                    entry = {k: v for k, v in r.items() if k != '_failed_fields'}
                    f.write(json.dumps(entry, ensure_ascii=False) + '\n')
                    updated += 1
    print(f"[PROFILES] 更新 {updated} 条，涉及 {len(by_wxid)} 个用户")
