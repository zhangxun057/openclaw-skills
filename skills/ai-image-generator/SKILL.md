...
description: |
  ...
  - 信息搜索与创意酝酿（博查搜索 → 提炼视觉细节 → 注入 Prompt）
  - 参考图搜索与视觉验证（多方案搜索 + qwen-vl-plus）
...
【Step 1: AI 理解需求】→ 调用大模型分析语义
    ├─ 场景类型识别
    ├─ 参考图需求判断
    └─ 创意方向生成（含 search_needed 搜索命题）
    ↓
【Step 2: 信息搜索与创意酝酿】→ 博查搜索 + AI 提炼
    ├─ 执行 search_needed 命题
    ├─ 提取关键视觉细节（不是罗列结果）
    └─ 将真实细节注入创意方向，深化 Prompt
    ↓
【Step 2: 参考图搜索（如需要）】
    ├─ 多方案搜索参考图
    └─ qwen-vl-plus 视觉验证
...
## Step 2: 信息搜索与创意酝酿（核心环节）

**这是 Skill 的灵魂所在** —— 搜索不是目的，注入创意才是。

### 核心理念

信息搜索的价值不在于"搜到了什么"，而在于**将搜索结果转化为可执行的视觉语言**。

传统做法：搜索 → 列结果 → 交给设计师
本 Skill 做法：搜索 → **提炼视觉细节** → **注入创意 Prompt** → 直接生成

### 执行流程

```
Step 1 输出的 search_needed 命题（3-5 个）
    ↓
【A. 批量搜索】→ 博查搜索 API，每个命题搜一次
    ↓
【B. 细节提炼】→ AI 提取关键视觉信息
    ├─ 空间维度：场地布局、尺寸、比例
    ├─ 视觉维度：色彩、光线、材质、纹理
    ├─ 时间维度：季节、天气、时段（日出/黄昏/夜景）
    └─ 文化维度：标志性元素、品牌符号、传统纹样
    ↓
【C. 创意注入】→ 将细节写入创意方向，深化 Prompt
    └─ 不是"黄果树瀑布"，而是"77米落差的多级跌瀑，白色水帘+灰色岩壁+翠绿植被"
```
...
def search_bocha(query, count=5):
    """
    博查搜索 API，获取背景信息
    """
    headers = {
        "Authorization": "Bearer YOUR_BOCHA_API_KEY_HERE",
        "Content-Type": "application/json"
    }
...
```python
def gather_context(search_queries, dashscope_key):
    """
    对每个搜索命题执行搜索，然后用 AI 提炼视觉细节
    """
    # 1. 批量搜索
    all_results = {}
...
{{
    ...
    "confidence": "high/medium/low（搜索结果的可靠度）"
}}
"""
...
信息搜索完成后，将提炼的视觉细节注入到 Step 4 的生成 Prompt 中：

**注入前（原始 Prompt）**：
```
某明星站在黄果树大瀑布前，手持某品牌酒瓶
```

**注入后（有依据的 Prompt）**：
```
某明星站在黄果树大瀑布前，瀑布高 77 米，白色水流从多级灰色岩壁跌落，
水雾弥漫形成彩虹；手持红色飘带、白色瓷瓶的某品牌酒瓶；
背景翠绿植被环绕，前景水花飞溅反射阳光
```

**关键原则**：
- ✅ 用搜索结果的**具体视觉描述**替换泛泛而谈
- ✅ 数字、颜色、材质、尺寸要具体
- ❌ 不要把搜索结果原文抄进 Prompt（那是文本，不是视觉语言）
- ❌ 不要把不相关的事实堆砌进去
...
| 文件 | 用途 | 何时使用 |
|------|------|----------|
| `scripts/step2_search.py` | 博查搜索 + 视觉细节提炼 | 信息搜索（Step 2） |
| `scripts/step3_ref_search.py` | 参考图搜索 + 下载 | 参考图搜索（Step 3） |
| `scripts/step3_ref_verify.py` | 视觉验证（qwen-vl-plus） | 参考图验证（Step 3） |
| `scripts/step3_5_ref_analyze.py` | 图片质量分析 | 参考图筛选（Step 3） |
| `scripts/step4_generate.py` | 图像生成流程 | 生成图片（Step 4） |
| `scripts/image2_tool.py` | GPT-Image-2 API 封装 | 图像生成底层 |
| `scripts/serper_tool.py` | Serper 图片搜索 | 参考图搜索（备选） |
| `scripts/vision_tool.py` | qwen-vl-plus 视觉验证 API | 视觉验证底层 |
...
| 问题 | 解决方案 |
|------|----------|
| 信息搜索无结果 | 检查博查 API Key 和网络连接，或精简搜索命题 |
